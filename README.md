...
- `@bradheitmann/odin-sentinel@0.7.0` is installed or available through a pinned
...
```bash
pnpm dlx --package @bradheitmann/odin-sentinel@0.7.0 odin-sentinel-mcp
```

Supported npm global install:

```bash
npm i -g @bradheitmann/odin-sentinel@0.7.0
```

Supported npx zero-install path:

```bash
npx -y -p @bradheitmann/odin-sentinel@0.7.0 odin-sentinel-mcp
```
...
A successful response includes `"serverInfo":{"name":"odin-sentinel","version":"0.7.0"}`.
...
```bash
claude mcp add odin-sentinel -- pnpm dlx --package @bradheitmann/odin-sentinel@0.7.0 odin-sentinel-mcp
```
...
command = "pnpm"
args = ["dlx", "--package", "@bradheitmann/odin-sentinel@0.7.0", "odin-sentinel-mcp"]
```
...
```json
{
  "mcp": {
    "odin-sentinel": {
      "type": "local",
      "command": ["pnpm", "dlx", "--package", "@bradheitmann/odin-sentinel@0.7.0", "odin-sentinel-mcp"],
      "enabled": true
    }
  }
}
