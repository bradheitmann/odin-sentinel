...
const packageJson = {
  ...
  odin: { publicVersion: "0.4.5", minimumCompatibleChildMcpVersion: "0.4.5" }
};

const scpText = "SCP_PUBLIC_VERSION: 0.4.5\nMIN_COMPATIBLE_CHILD_MCP: 0.4.5\n";
...
function runWith(overrides: Record<string, unknown> = {}) {
  const publicVersionFiles = {
    "README.md": "0.4.5",
    "docs/guides/quick-start.md": "0.4.5",
    "docs/guides/quickstart-prompts.md": "0.4.5",
    "docs/reference/client-compatibility.md": "Node.js >=22.13.0",
    "docs/reference/distribution.md": "0.4.5",
    "docs/reference/public-surface-audit.md": "0.4.5",
    ...
    "plugins/sentinel-coordination-protocol/README.md": "MCP tools: 23 `odin.*` tools",
  ...
  return verifyPack.runVerifyPack({
    pack: { files: requiredPaths, filename: "odin-sentinel-0.4.5.tgz" },
    packageJson,
    publicVersionFiles,
...
describe("release sync audit helpers", () => {
  it("accepts synchronized public package artifacts without inspecting private skill copies", () => {
    expect(runWith().version).toBe("0.4.5");
  });
  ...
  it("rejects stale versions in packaged protocol resources", () => {
    expect(verifyPack.validatePackagedProtocolVersions({
      ...
      "protocol/closeout.yaml": "version: 0.4.5\ncloseout: {}"
    }, "0.4.5")).toEqual([
      "protocol/model-profiles.yaml: protocol resource version 0.2.1 must match package version 0.4.5",
      "protocol/SCP.md: protocol resource version 0.3.0 must match package version 0.4.5"
    ]);
  });

  it("rejects repo-internal public protocol drift", () => {
    expect(() => runWith({ publicVersionFiles: { "protocol/bootstrap-skill.md": bootstrapText.replace("0.4.5", "0.4.4") } })).toThrow(/protocol\/bootstrap-skill\.md missing SCP_PUBLIC_VERSION: 0\.4\.5/);
  });

  it("rejects stale public version references", () => {
    expect(verifyPack.findStaleVersionReferences({ "docs/reference/distribution.md": "server 0.2.1" }, "0.4.5")).toEqual([
      "docs/reference/distribution.md: stale version reference 0.2.1"
    ]);
  });

  it("requires public package metadata for consumers", () => {
    expect(verifyPack.validatePackageMetadata({ version: "0.4.5", files: [] })).toContain("package.json missing repository.url");
  });
  ...
  it("rejects Claude plugin manifest and skill drift", () => {
    expect(() => runWith({
      publicVersionFiles: {
        ...
        "plugins/sentinel-coordination-protocol/skills/sentinel-coordination-protocol/SKILL.md": "SCP_PUBLIC_VERSION: 0.4.4\n",
        "plugins/sentinel-coordination-protocol/README.md": "MCP tools: 17 odin tools"
      }
    })).toThrow(/Claude plugin manifest version 0\.4\.4 must match package version 0\.4\.5/);
  });
