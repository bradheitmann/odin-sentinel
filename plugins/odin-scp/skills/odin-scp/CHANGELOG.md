...
- Release check: `scripts/audit/verify-pack.mjs` no longer checks a changelog's sections for earlier releases for unpinned install references. An earlier section's publication note names the version it shipped and no longer fails a later release. Every other line is still checked, including the current section, later-version sections, and text under non-release headings.
