...
- A native target that is `$HOME` or `/` (a line holding only a tilde and a slash, the absolute home path, or any spelling that resolves to either) stops the run in every mode before anything is written, whether or not master is inside `$HOME`. A proper ancestor of `$HOME` is refused per target in write mode and `--dry-run` (one that also contains master is skipped by the master-overlap guard first); ancestors of the physical location are included, so a `$HOME` that is a symlink still protects the directories that really contain it.
