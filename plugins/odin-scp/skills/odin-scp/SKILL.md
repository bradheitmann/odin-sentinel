...
- Keep agents interchangeable by role, not by blurred authority. Any supported harness may serve any role only after it has a clean boot block declaring the current role, write scope, branch, cwd, model/harness, and proof source. The same assignment must not QA and close its own work. Closure independence is machine-enforced with named refusals, not an honor-system rule; see the closure-independence amendment under Role And QA Hard Gates.
...
- `QA WORKER`: independent adversarial QA inside exact review scope. Cannot QA own work or self-close; enforced by the named closure refusals in Role And QA Hard Gates, not by convention.
...
Fallback contracts must pin the MODEL, not just the harness: a harness's default model is chosen for the harness's general use, not for the role, and a control-plane seat inheriting a non-agentic default fails silently — the pod just stalls. Declare each rung as harness + required model + flags. If no authorized substitute exists for a role, the declared fallback is PAUSE_ESCALATE (pause the pod, escalate to the operator), never improvisation. Provider billing errors ("insufficient balance… retrying") are NOT substitution triggers — they are operator-side; hold the seat. After every relaunch or account change, re-verify the committed model from the status bar before trusting the seat (model identity drifts on relaunch). For provider-wide mass outages, emit an `[SCP-OUTAGE-HANDOFF]` receipt BEFORE going dark: affected provider/account, every affected slot, in-flight work + last-safe state, critical roles needing cover, a provider-diverse surviving continuity seat, and a bounded expiry — the survivor opens a time-boxed `[SCP-EXCEPTION]`, staffs cover from UNAFFECTED seats only, and reverts to canonical roles the moment the account recovers. Validate with `odin.validate_fallback_contract` / `odin.validate_successor_contract` / `odin.validate_outage_handoff`.
...
Submit semantics are per-harness, and Enter alone is not enough. Each harness declares a `submit_profile`: `single_line_flatten` (the harness submits on EVERY embedded newline — compose the ENTIRE message as ONE line, flatten newlines/CR/tabs to single spaces, and join fields with a separator such as `" ;; "`; a multi-line message fires one prompt per line, jamming the recipient and burning its context), `double_enter` (standard send plus a second Enter; a long paste during a busy transition may need the second Enter), or `single_enter_verify` (exactly one Enter, then read-screen verify; a blind second Enter interrupts reply generation — re-Enter ONCE only if the input bar still shows the message). The harmful trigger for single-line harnesses is the embedded newline itself, not the number of send calls; a "complete message" that is multi-line still machine-guns. Before dispatching to another agent, check the recipient's format profile (harness-pacing seeds) and bake it into the delegation. Run a pre-dispatch dry-run (`dry_run: true`, `target_submit_profile`, payload) through `odin.validate_cmux_delivery_proof`: a multi-line payload aimed at a `single_line_flatten` harness fails as `MULTILINE_TO_SINGLE_SUBMIT_HARNESS` even with `submitted: true`, unless the canonical send helper auto-flattened it (`newlines_flattened: true`).
...
<!-- BEGIN SCP-AMENDMENT GOVEDGE-HARNESS-IDENTITY-UNIQUENESS -->
**Doctrine amendment (GOVEDGE-HARNESS-IDENTITY-UNIQUENESS): harness identity is unique at load and canonical-first at resolution (source: STORY-GOVEDGE-S3).** Harness recipe data loads fail-closed: a duplicate harness_id across entries is refused as `duplicate_harness_id`; a duplicate display_name across entries is refused as `duplicate_harness_display_name`; any display_name byte-equal to ANY entry's harness_id is refused as `display_name_collides_with_harness_id`; all uniqueness and collision comparisons are exact byte comparisons with no folding of any kind; resolution consults the canonical id channel BEFORE the display-name channel; the single shared canonical-id pattern is the stricter runtime form; a malformed id refuses at load as `non_canonical_harness_id` regardless of which harness is probed; onboarding classifies an unresolvable requested harness as a named row while the probe-matrix surface stays whole-batch fail-closed.
<!-- END SCP-AMENDMENT GOVEDGE-HARNESS-IDENTITY-UNIQUENESS -->
